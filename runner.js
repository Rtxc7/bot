...
number = '67579764981@s.whatsapp.net'
...
zero.sendMessage(anu.jid, "Hai Saya Felix!\nSilakan Kirim #menu Untuk Melihat Menu Bot😊.", "conversation")
}
...
sendButImage(anu.jid, teks, `Welcome Message By Felix-Chan`, img,but = [{buttonId:`hallo`, 
...
img = await getBuffer(ppUrl)
teks = `Bye Bye @${num.split('@')[0]}\nKeluar Dari Group: ${mdata.subject}`
sendButImage(anu.jid, teks, `Leave Message By Felix-Chan`, img,but= [{buttonId: `byebye`, buttonText: {displayText: `SELAMAT TINGGAL`}, type: 1}], options = {contextInfo: {mentionedJid: [num, number]}, thumbnail: Buffer.alloc(0)})
}
